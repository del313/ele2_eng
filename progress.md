...
- [ ] **Grade 5 (小五)**：15單元 (進行中) - 已完成 15 單元教材產生與影片驗證，索引頁面 `g5/index.html` 已建立，遊戲庫開發中。
- [ ] **Grade 6 (小六)**：18單元 (規劃中) - 學術準備，重點在於不規則動詞三態與連接詞。

### 3. 📱 平台優化 (Platform Optimization)
- [x] **全域進度邏輯修復**：修正 `index.html` 的進度計算 Bug，現在能正確讀取物件格式的進度資料。
- [x] **G3/G5 課程擴充**：
    - [x] 完成 G3 8 款遊戲整合與 12 單元發布。
    - [x] 完成 G5 `vocab_g5.json` (15 單元) 資料庫建立。
    - [x] 完成 G5 15 個 HTML 單元自動化產生與 100% 影片有效性驗證。
- [x] **UI/UX 深度修復**：
